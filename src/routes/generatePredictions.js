...
async function generatePrompt(
  ...
  try {
    ...
    if (categories === "on_model") {
      ...
      contentMessage = `Write a very long prompt in English that provides a highly detailed and vivid description of the item. Imagine a creative photoshoot scene for this product, focusing on highlighting the item with captivating angles and an atmosphere that draws the viewer in. Begin by setting the scene: describe the environment in exquisite detail, such as the way sunlight filters through the leaves of a lush garden, casting dappled light on the product, or the soft shadows in a modern, minimalist studio. Explain how this setting complements the product, crafting a visual narrative that engages the audience's attention.\n\nDescribe every aspect of the item meticulously. For instance, if it is a flowing white dress, detail how the fabric catches the light, emphasizing its shimmering lace and how it drapes elegantly over the model’s figure. Include every intricate pattern and subtle detail that makes the item unique, using sensory language to make these characteristics come to life vividly.\n\n${
        environmentContext
          ...
          : ""
      }\n\nBring the environment to life with rich sensory details: describe the play of light and shadow, the textures of the surroundings, and how these elements interact with the product. Paint a vivid picture of whether the product blends seamlessly into the scene or stands out as a striking focal point. Elaborate on how the product feels to the touch, how it moves, and how its colors change under different lighting conditions. Use language that effectively conveys the mood and setting to evoke emotions and spark the viewer's imagination.\n\n${
        extraPromptDetail
